...
void Proximity(int index, const std::vector<std::vector<float>> &points, std::vector<int> &cluster, std::vector<bool> &processed_indices, KdTree *tree, float distanceTol)
{
	processed_indices[index] = true;
	cluster.push_back(index);

	std::vector<int> nearest_points = tree->search(points[index], distanceTol);
	for (auto i : nearest_points)
	{
		if (!processed_indices[i])
		{
			Proximity(i, points, cluster, processed_indices, tree, distanceTol);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	std::vector<bool> processed_indices(points.size(), false);
	...
	while (index < points.size())
	{
		if (!processed_indices[index])
		{
			...
			Proximity(index, points, cluster, processed_indices, tree, distanceTol);
